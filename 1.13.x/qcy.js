...
config.outbounds.forEach(i => {
  ...
  if (['GLOBAL'].includes(i.tag)) {
    ...
    const globalProxies = proxies.filter(proxy => !excludeRegex.test(proxy.tag))
    const targets = globalProxies.length > 0 ? globalProxies : proxies
    i.outbounds.push(...getTags(targets))
  }
})
